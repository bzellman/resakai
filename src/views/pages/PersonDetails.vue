...
onMounted(async () => {
    ...
    if (personsStore.users.length > 0) {
        person.value = personsStore.users[0]; // Assume only one entry per user
    }
});
...
function savePerson() {
    ...
    if (person.value.name.trim()) {
        if (person.value.id) {
            personsStore.saveUser(person.value);
...
<template>
    <div>
        <div class="card">
            ...
            <div v-else-if="personsStore.users.length === 0" class="flex flex-col items-center">
